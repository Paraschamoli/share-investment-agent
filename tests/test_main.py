...
from share_investment_agent.main import handler
...
async def test_handler_returns_response():
    ...
    with patch("share_investment_agent.main._initialized", True), \
...
async def test_handler_with_multiple_messages():
    ...
    with patch("share_investment_agent.main._initialized", True), \
         patch("share_investment_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response) as mock_run:
        result = await handler(messages)
...
async def test_handler_initialization():
    ...
    with patch("share_investment_agent.main._initialized", False), \
